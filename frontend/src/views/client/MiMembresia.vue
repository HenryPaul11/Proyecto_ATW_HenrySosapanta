...
const esActiva = computed(() => (store.membresia?.estado || '').toUpperCase() === 'ACTIVA')
...
const porcentajeUsado = computed(() => {
  if (!store.membresia) return 0
  const inicio = new Date(store.membresia.fechaInicio).getTime()
  const fin    = new Date(store.membresia.fechaFin).getTime()
    ...
    <main class="flex-1 w-full max-w-3xl mx-auto px-4 sm:px-8 py-8 md:py-10 fade-in">
      ...
      <template v-else>
        ...
        <div class="rounded-2xl p-6 mb-5 border-2"
          :class="esActiva ? 'bg-emerald-50 border-emerald-300' : 'bg-red-50 border-red-300'">
          <div class="flex items-center justify-between flex-wrap gap-3">
            <div>
              <p class="text-xs font-bold uppercase tracking-widest mb-1"
                :class="esActiva ? 'text-emerald-600' : 'text-red-600'">
                Estado de membresía
              </p>
              <h2 class="text-2xl font-black"
                :class="esActiva ? 'text-emerald-800' : 'text-red-800'">
                {{ store.membresia.plan?.nombrePlan }}
              </h2>
              <p class="text-sm mt-1"
                :class="esActiva ? 'text-emerald-700' : 'text-red-700'">
                {{ store.membresia.plan?.descripcion }}
              </p>
            </div>
            <span class="px-4 py-2 rounded-full font-black text-sm uppercase tracking-wide"
              :class="esActiva ? 'bg-emerald-500 text-white' : 'bg-red-500 text-white'">
        ...
        <div class="bg-white rounded-2xl border border-slate-200 shadow-sm p-6 mb-5">
          ...
          <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div class="bg-slate-50 rounded-xl p-4">
              ...
              <p class="text-2xl font-black text-emerald-600">${{ (store.membresia.plan?.precio ?? 0).toFixed(2) }}</p>
            ...
            <div class="bg-slate-50 rounded-xl p-4">
              ...
              <p class="text-2xl font-black" :class="diasRestantes <= 7 ? 'text-amber-500' : 'text-blue-600'">
                {{ esActiva ? diasRestantes : '—' }}
              </p>
              <p class="text-xs text-slate-400 mt-0.5">{{ esActiva ? 'días' : 'membresía vencida' }}</p>
            </div>
            <div class="bg-slate-50 rounded-xl p-4">
              <p class="text-xs font-bold text-slate-400 uppercase tracking-wider mb-1">Fecha de inicio</p>
              <p class="text-base font-bold text-slate-800">{{ formatFecha(store.membresia.fechaInicio) }}</p>
            </div>
            <div class="bg-slate-50 rounded-xl p-4">
              <p class="text-xs font-bold text-slate-400 uppercase tracking-wider mb-1">Fecha de vencimiento</p>
              <p class="text-base font-bold text-slate-800">{{ formatFecha(store.membresia.fechaFin) }}</p>
            </div>
          </div>
        </div>

        <!-- Alertas -->
        <div v-if="esActiva && diasRestantes <= 7"
        ...
        <!-- Barra de progreso -->
        <div v-if="esActiva" class="bg-white rounded-2xl border border-slate-200 shadow-sm p-6">
          <div class="flex justify-between text-xs font-semibold text-slate-500 mb-2">
            <span>{{ formatFecha(store.membresia.fechaInicio) }}</span>
            <span>{{ porcentajeUsado }}% usado</span>
            <span>{{ formatFecha(store.membresia.fechaFin) }}</span>
          </div>
